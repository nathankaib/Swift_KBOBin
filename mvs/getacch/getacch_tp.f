...
c Last revision: 2/24/94
     ...
     &     xht,yht,zht,istat,axht,ayht,azht,time)
...
c..   do direct and indirect force from Sun on fixed orbit
      call fixedsun_acc_tp(ntp,istat,xht, yht, zht,a_sun_x,a_sun_y,
     &     a_sun_z,time,mass)

c...  add them all together
      do i=1,ntp
         if(istat(i,1).eq.0) then
            axht(i) = axh0 + axh3(i) + a_sun_x(i)
            ayht(i) = ayh0 + ayh3(i) + a_sun_y(i)
            azht(i) = azh0 + azh3(i) + a_sun_z(i)
         endif
      enddo
