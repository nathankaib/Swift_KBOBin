...
c...  Inputs: 
      ...
      real*8 xh(NPLMAX),yh(NPLMAX),zh(NPLMAX),time
...
c..   do direct and indirect force from Sun on fixed orbit
c      call fixedsun_indirect(a_sun_x,a_sun_y,
c     &     a_sun_z,time,mass)

c..   note: i'm ignoring the direct force from Sun on passing KBOs 
c..   because the difference in this term between passing KBOs and
c..   the binary primary is tiny, and the passages are artificially 
c..   stopped nad started anyways
...
c...  add them all together
      ...
      azh(1) = 0.0
      do i=2,nbod
        axh(i) = axh0 + a_sun_x + axh1(i) + axh2(i) + axh3(i)
        ayh(i) = ayh0 + a_sun_y + ayh1(i) + ayh2(i) + ayh3(i)
        azh(i) = azh0 + a_sun_z + azh1(i) + azh2(i) + azh3(i)
      enddo
