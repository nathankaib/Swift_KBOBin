...
c Last revision: 1/20/97
      ...
      subroutine discard(time,dt,nbod,npl,ntp,mass,xh,yh,zh,vxh,
...
c...  Executable code 
      ...
      if( (rmin.ge.0.0) .or. (rmax.ge.0.0) .or. (rmaxu.ge.0.0) ) then
         call coord_h2b(npl,mass,xh,yh,zh,vxh,vyh,vzh,
...
c...  not checking for close pl encounters because we only have KBO passages
c      if(lclose) then
c         call discard_pl(time,dt,nbod,ntp,mass,xh,yh,zh,vxh,vyh,vzh,
c     &       xht,yht,zht,vxht,vyht,vzht,rplsq,istat,rstat)
c      endif
